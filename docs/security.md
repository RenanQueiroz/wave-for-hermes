...
- Tool calls render only as bounded one-line actions derived from validated tool names and
  ...
  not displayed.
- Reasoning renders as one bounded, explicitly truncated trace per assistant message,
  normalized from the gateway's plain-text reasoning fields and `reasoning.delta` frames and
  rendered through the same `Response` markdown pipeline as assistant text — model-authored
  prose under the same link-scheme allowlist, never executed. Opaque provider reasoning
  structures never enter the render model, and Realtime voice stores and displays no reasoning.
