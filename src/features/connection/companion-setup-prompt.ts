...
3. Make it reachable from my phone through Tailscale. Install and authenticate Tailscale if it is missing — if that requires opening a URL or an auth key, tell me exactly what to do. Prefer "tailscale serve --bg 8787" so the companion gets a real HTTPS hostname. If Tailscale Serve is unavailable on my tailnet, serving plain HTTP on the machine's tailnet address is acceptable: the Wave app allows http:// for Tailscale 100.64.0.0/10 addresses, where the tunnel already encrypts the traffic. As a fallback for a trusted home network only, the app also accepts an explicitly typed http:// URL to this machine's private LAN address (or its name.local on iOS), but that traffic is unencrypted on the network, so mention it only if Tailscale is not an option. Never expose the companion to the public internet.
